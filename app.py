...
from query_expander import QueryExpander, ParaphrasedQuery
from bs4 import BeautifulSoup
import requests
...
def process_website(url):
    try:
        ...
        documents = processUrlWithBeautifulSoup(url)
    
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP,separators=["\n\n", "\n", " ", ""])
    texts = text_splitter.split_documents([documents])
    st.write(f"**Number of text chunks after splitting:** {len(texts)}")
    for i, d in enumerate(texts[:3], start=1):
        print(f"--- Chunk {i} ---")
        print(d.page_content[:200], "...\n")
    return texts

def processUrlWithBeautifulSoup(url):
    st.write(f"Processing url with beautifulsoup")
...
def rag_pipeline(query, qa_chain, vectorstore):
    ...
    # expand the query using QueryExpander
    expander = QueryExpander()
    expanded_queries = expander.expand_query(query)
    st.write("\n**Expanded Queries:**")
    for i, variation in enumerate(expanded_queries, 1):
...
Answer the question concisely based only on the given context. If the context doesn't contain relevant information, consider it as a general question and respond accordingly."
